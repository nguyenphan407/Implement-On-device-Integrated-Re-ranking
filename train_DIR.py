...
import random
import argparse
import json  # nếu cần sử dụng json để load file stat
...
# Tắt eager execution để code chạy theo mode đồ thị như TF1
...
def train(args):
    ...
    model_name = '{}_DIR_{}_{}_{}_{}_{}_{}_{}_{}_vec2'.format(
        args.timestamp, args.batch_size, args.lr, args.l2_norm, args.emb_dim,
        args.keep_prob, args.hidd_size, args.expert_num, args.n_head)
    save_dir_path = '{}/{}/'.format(args.save_dir, model_name)
    if not os.path.exists(save_dir_path):
        os.makedirs(save_dir_path)
    save_path = '{}/{}/ckpt'.format(args.save_dir, model_name)
    metrics_save_path = '{}/{}.metrics'.format(args.save_dir, model_name)
    model_save_path = '{}/{}.pkl'.format(args.save_dir, model_name)
    ...
    with tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        for epoch in range(args.epoch_num):
            ...
            for batch_no in range(batch_num):
                ...
                loss_val = model.train(batch, split_hist=split_hist, union_hist=union_hist)

                step += 1
                train_losses_step.append(loss_val)
                ...
                if step % eval_iter_num == 0:
                    ...
                    print("EPOCH %d  STEP %d  LOSS TRAIN: %.4f | LOSS VALI: %.4f   AUC: %.4f JS: %.4f" %
                          (epoch, step, train_loss, vali_loss, auc, res[0]))
                    for i, scope in enumerate(args.metric_scopes):
                        print("\t@%d  MAP: %.4f  NDCG: %.4f  CLICK: %.4f" % (scope, res[1][i], res[2][i], res[3][i]))
                    if res[0] > best_util:
                        best_util = res[0]
                        ...
                        with model.graph.as_default():
                            # Giả sử bạn có self.sess, và đầu vào là tensor `x_ph` đầu ra `y_pred`
                            model.export_saved_model(
                                sess=sess,
                                export_dir="/path/to/saved_model",
                                input_tensors={"input": model.x_ph},
                                output_tensors={"output": model.y_pred}
                            )
                        early_stop = False
                        continue
                    ...
                    if len(training_monitor['utility']) > 2 and epoch > 0:
                        if epoch > 40 and best_util - training_monitor['utility'][-1] > 0.08:
                            early_stop = True
                        # Các điều kiện early stop khác có thể được kích hoạt tại đây.
...
def reranker_parse_args():
    ...
    parser.add_argument('--eval_freq', type=int, default=4, help='the frequency of evaluating on the valid set when training')
