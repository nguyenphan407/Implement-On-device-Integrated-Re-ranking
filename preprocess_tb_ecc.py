...
class Processor:
    ...
    def __init__(self,train_path, test_path, processed_path):
    ...
    def load_data(self, data_path):
        ...
        for line in csv_reader:
            ...
            if label:
                pos_item_hist[idx] = self.get_hist(line[18:24])
        ...
        self.stats(user_ft, user_pos_item, user_neg_item, item_ft, pos_item_hist)
    ...
    def process_data(self):
        ...
        if os.path.exists(self.processed_path + 'train.filter'):
            train_u_ft, train_u_pos_itm, train_u_neg_itm, train_i_ft, train_pos_i_hist = pkl.load(open(self.processed_path + 'train.filter', 'rb'))
        else:
            train_u_ft, train_u_pos_itm, train_u_neg_itm, train_i_ft, train_pos_i_hist = self.load_data(self.train_path)
            pkl.dump([train_u_ft, train_u_pos_itm, train_u_neg_itm, train_i_ft, train_pos_i_hist], open(self.processed_path + 'train.filter', 'wb'))
        print('load test dataset')
        if os.path.exists(self.processed_path + 'test.filter'):
            test_u_ft, test_u_pos_itm, test_u_neg_itm, test_i_ft, test_pos_i_hist = pkl.load(open(self.processed_path + 'test.filter', 'rb'))
        else:
            test_u_ft, test_u_pos_itm, test_u_neg_itm, test_i_ft, test_pos_i_hist = self.load_data(self.test_path)
            pkl.dump([test_u_ft, test_u_pos_itm, test_u_neg_itm, test_i_ft, test_pos_i_hist], open(self.processed_path + 'test.filter', 'wb'))
        ...
        train_set = self.generate_dataset(train_u_ft, train_u_pos_itm, train_u_neg_itm, train_i_ft, train_pos_i_hist,
                         uid_map, usr_ft_map, itm_ft_map)
        test_set = self.generate_dataset(test_u_ft, test_u_pos_itm, test_u_neg_itm, test_i_ft, test_pos_i_hist,
                         uid_map, usr_ft_map, itm_ft_map)
        ...
        print('stat', stat)
        with open(self.processed_path + 'data.stat', 'w') as f:
            stat = json.dumps(stat)
            f.write(stat)
        for postfix, date in [('train', train_set), ('test', test_set)]:
            with open(self.processed_path + 'data.' + postfix, 'wb') as f:
                pkl.dump(date, f)
